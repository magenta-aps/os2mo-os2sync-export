...
def filter_org_unit(
    org_unit: ReadUserItusersObjectsCurrentEngagementOrgUnit,
    top_unit_uuid: UUID,
    hierarchy_user_keys: list[str],
    filtered_org_units: list[UUID],
    ignored_org_unit_level: list[UUID],
    ignored_unit_type: list[UUID],
):
    ancestors = {o.uuid for o in org_unit.ancestors}
    if org_unit.uuid == top_unit_uuid:
        return True
    if (
        org_unit.org_unit_level in ignored_org_unit_level
        or org_unit.unit_type in ignored_unit_type
    ):
        return False
    if org_unit.uuid in filtered_org_units or ancestors.intersection(
        set(filtered_org_units)
    ):
        return False
    if (
        hierarchy_user_keys
        and org_unit.org_unit_hierarchy_model
        and org_unit.org_unit_hierarchy_model.user_key not in hierarchy_user_keys
    ):
        return False
    return True
...
async def read_person_accounts(
    ...
    for f in ad_accounts:
        ...
        if current is None or current.engagement is None:
            yield fk_uuid, None
        assert current and current.engagement  # only to help mypy
        current.engagement = [
            e
            for e in current.engagement
            if filter_org_unit(
                one(e.org_unit),
                top_unit_uuid,
                hierarchy_user_keys,
