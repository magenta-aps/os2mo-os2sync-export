...
from os2sync_export.__main__ import delete_all_from_fk_org
...
@fastapi_router.post("/cleanup_delete_all")
async def trigger_delete_all(
    settings: Settings_,
    os2sync_client: OS2SyncClient_,
    dry_run: bool = False,
):
    """Deletes every org_unit and user from FK-org.

    After this completes, manually clear the OS2Sync success tables,
    then trigger a full re-sync of all events from MO.
    """
    client = get_os2sync_client(settings=settings, session=None, dry_run=dry_run)
    await delete_all_from_fk_org(os2sync_client=client)
    return Response(
        content=textwrap.dedent("""\
            All users and org units have been deleted from FK-org.
            To reimport, follow these steps:

            1. Connect to OS2Sync's database:
               MYSQL_PASSWORD=$(cat /opt/docker/os2sync_4/docker-compose.yml | yq --raw-output '.services.mysql.environment.MYSQL_PASSWORD')
               docker exec -it os2sync_4-mysql-1 mysql -u os2sync -p$MYSQL_PASSWORD os2sync

            2. Wait for OS2Sync to finish processing deletes:
               SELECT COUNT(*) FROM queue_users;
               SELECT COUNT(*) FROM queue_orgunits;

            3. Clear the success tables:
               DELETE FROM success_users;
               DELETE FROM success_orgunits;

            4. Trigger a full re-sync of all events from MO.
        """),
