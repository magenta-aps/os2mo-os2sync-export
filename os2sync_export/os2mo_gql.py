...
def convert_to_os2sync(
    ...
    positions = [
        Position(
            ...
            if settings.use_extension_field_as_job_function and i.current.extension_3
            else i.current.job_function_response.current.name,  # type: ignore
            OrgUnitUuid=UUID(
                first(
                    i.current.org_unit_response.current.itusers_response.objects
                ).current.user_key  # type: ignore
            )
            if i.current.org_unit_response.current.itusers_response.objects
            else i.current.org_unit_response.current.uuid,
        )
