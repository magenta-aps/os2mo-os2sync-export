...
def convert_to_os2sync(
    ...
    # ou_filter = partial(filter_relevant_orgunit, settings=settings)
    # engagements = filter(ou_filter, it.engagement) if it.engagement else []
    ...
    positions = [
        ...
        if filter_relevant_orgunit(settings, one(i.org_unit))
    ]
...
def filter_relevant_orgunit(
    settings: Settings,
    orgunit_data: ReadOrgunitOrgUnitsObjectsCurrent
    | ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementOrgUnit,
) -> bool:
