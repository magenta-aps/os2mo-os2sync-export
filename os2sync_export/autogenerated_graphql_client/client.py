...
class GraphQLClient(AsyncBaseClient):
    ...
    async def locate_f_k_user(self, user_keys: str, uuid: UUID) -> LocateFKUser:
        query = gql(
            """
            query LocateFKUser($user_keys: String!, $uuid: UUID!) {
        ...
        variables: dict[str, object] = {"user_keys": user_keys, "uuid": uuid}
        response = await self.execute(query=query, variables=variables)
        data = self.get_data(response)
        return LocateFKUser.parse_obj(data)
