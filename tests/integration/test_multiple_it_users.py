...
async def test_read_person(
        ...
        call(
            User(
                ...
                Positions=[
                    Position(
                        Name="Tester",
                        OrgUnitUuid=create_engagements[0].org_unit,
                        StartDate=None,
                        StopDate=None,
                    )
                ],
    ...
    # Sort both result and expected by uuid to ensure they are in the same order.
    assert sorted(
        os2sync_mock.update_user.call_args_list, key=lambda x: x.Uuid
    ) == sorted(expected, key=lambda x: x.Uuid)
