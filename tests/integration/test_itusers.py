...
from os2sync_export.autogenerated_graphql_client import FacetCreateInput
...
async def test_account_with_addresses(
    ...
    # find address types:
    address_facet = (
        await graphql_client.testing__create_facet(
            input=FacetCreateInput(
                user_key="employee_address_type",
                validity=ValidityInput(from_=None, to=None),  # type: ignore
            )
        )
    ).uuid

    email_address_type_uuid = (
        await graphql_client.testing__create_class(
            input=ClassCreateInput(
                name="email",
                user_key="email",
                scope="EMAIL",
                facet_uuid=address_facet,
                validity=ValidityInput(from_=None, to=None),  # type: ignore
            )
        )
    ).uuid

    mobile_address_type_uuid = (
        await graphql_client.testing__create_class(
            input=ClassCreateInput(
                name="Mobile",
                user_key="mobile",
