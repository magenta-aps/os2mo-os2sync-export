# SPDX-FileCopyrightText: Magenta ApS
#
# SPDX-License-Identifier: MPL-2.0
from datetime import UTC
from datetime import datetime
from datetime import timedelta
from unittest.mock import AsyncMock
from unittest.mock import call
from unittest.mock import patch
from uuid import UUID
from uuid import uuid4

import pytest
from fastramqpi.pytest_util import retrying
from httpx import AsyncClient
from more_itertools import first
from more_itertools import one
from tenacity import stop_before_delay

from os2sync_export.autogenerated_graphql_client import GraphQLClient
from os2sync_export.autogenerated_graphql_client.input_types import AddressCreateInput
from os2sync_export.autogenerated_graphql_client.input_types import (
    EngagementCreateInput,
)
from os2sync_export.autogenerated_graphql_client.input_types import (
    EngagementUpdateInput,
)
from os2sync_export.autogenerated_graphql_client.input_types import ITUserCreateInput
from os2sync_export.autogenerated_graphql_client.input_types import RAValidityInput
from os2sync_export.autogenerated_graphql_client.read_user_i_t_accounts import (
    ReadUserITAccountsEmployeesObjectsCurrentFkOrgUuids,
)
from os2sync_export.config import Settings
from os2sync_export.os2mo_gql import read_fk_users_from_person
from os2sync_export.os2mo_gql import sync_mo_user_to_fk_org
from os2sync_export.os2sync_models import Person
from os2sync_export.os2sync_models import Position
from os2sync_export.os2sync_models import User


@pytest.mark.integration_test
async def test_event(
    test_client: AsyncClient,
    graphql_client: GraphQLClient,
    create_person,
    create_engagements,
    create_AD_itsystem,
    create_FK_itsystem,
) -> None:
    # Arrange
    adguid = uuid4()
    person_uuid = create_person.uuid

    # Act
    # This creates an event that the integration reacts to
    await graphql_client.testing__ituser_create(
        input=ITUserCreateInput(
            person=person_uuid,
            user_key="AD-username 1",
            external_id=str(adguid),
            engagements=[e.uuid for e in create_engagements],
            itsystem=create_AD_itsystem,
            validity=RAValidityInput(from_=datetime(1970, 1, 1), to=None),  # type: ignore
        )
    )
    # Check that only the AD account exists in MO
    fk_accounts, ad_accounts = await read_fk_users_from_person(
        graphql_client=graphql_client,
        uuid=person_uuid,
        it_user_keys=["Active Directory"],
    )
    assert len(ad_accounts) == 1
    assert fk_accounts == []

    # Assert
    async for attempt in retrying(stop=stop_before_delay(60)):
        with attempt:
            # Check that the fk-org account has been created
            fk_accounts, ad_accounts = await read_fk_users_from_person(
                graphql_client=graphql_client,
                uuid=person_uuid,
                it_user_keys=["Active Directory"],
            )
            fk_account = one(fk_accounts)
            assert UUID(fk_account.user_key) == adguid
            assert UUID(fk_account.external_id) == adguid


@pytest.mark.integration_test
async def test_account_with_addresses(
    graphql_client: GraphQLClient,
    create_AD_itsystem,
    create_FK_itsystem,
    create_person,
    create_engagements,
    create_email_address_type,
    create_mobile_address_type,
    create_landline_address_type,
    set_settings,
) -> None:
    # Test that emails are sent to fk-org with their correct account.
    # Arrange
    adguid1 = uuid4()
    adguid2 = uuid4()
    os2sync_mock = AsyncMock()
    person_uuid = create_person.uuid

    # AD users
    ad_user_1 = await graphql_client.testing__ituser_create(
        input=ITUserCreateInput(
            person=person_uuid,
            user_key="AD-username 1",
            external_id=str(adguid1),
            engagements=[e.uuid for e in create_engagements],
            itsystem=create_AD_itsystem,
            validity=RAValidityInput(from_=datetime(1970, 1, 1), to=None),  # type: ignore
        )
    )
    ad_user_2 = await graphql_client.testing__ituser_create(
        input=ITUserCreateInput(
            person=person_uuid,
            user_key="AD-username 2",
            external_id=str(adguid2),
            engagements=[e.uuid for e in create_engagements],
            itsystem=create_AD_itsystem,
            validity=RAValidityInput(from_=datetime(1970, 1, 1), to=None),  # type: ignore
        )
    )

    settings = set_settings(
        email_scope_classes=[create_email_address_type],
        phone_scope_classes=[create_mobile_address_type],
        landline_scope_classes=[create_landline_address_type],
    )
    # Create two email-addresses - one connected to each it-account
    await graphql_client.testing__address_create(
        input=AddressCreateInput(
            employee=create_person.uuid,
            value="bsg@example.com",
            address_type=create_email_address_type,
            ituser=ad_user_1.uuid,
            validity=RAValidityInput(from_=datetime(1970, 1, 1), to=None),  # type: ignore
        )
    )
    await graphql_client.testing__address_create(
        input=AddressCreateInput(
            employee=create_person.uuid,
            value="bsg2@example.com",
            address_type=create_email_address_type,
            ituser=ad_user_2.uuid,
            validity=RAValidityInput(from_=datetime(1970, 1, 1), to=None),  # type: ignore
        )
    )
    # Create a phone number for one it-account
    await graphql_client.testing__address_create(
        input=AddressCreateInput(
            employee=create_person.uuid,
            value="11223344",
            address_type=create_mobile_address_type,
            ituser=ad_user_1.uuid,
            validity=RAValidityInput(from_=datetime(1970, 1, 1), to=None),  # type: ignore
        )
    )
    # Create a landlines for the other it_account
    await graphql_client.testing__address_create(
        input=AddressCreateInput(
            employee=create_person.uuid,
            value="12345678",
            address_type=create_landline_address_type,
            ituser=ad_user_2.uuid,
            validity=RAValidityInput(from_=datetime(1970, 1, 1), to=None),  # type: ignore
        )
    )
    # Act
    await sync_mo_user_to_fk_org(
        uuid=person_uuid,
        graphql_client=graphql_client,
        settings=settings,
        os2sync_client=os2sync_mock,
    )
    # Assert
    assert os2sync_mock.update_user.call_count == 2
    os2sync_mock.delete_user.assert_not_called()

    expected = [
        call(
            User(
                Uuid=adguid1,
                ShortKey=None,
                UserId="AD-username 1",
                Person=Person(Name="Brian Graversen", Cpr=None),
                Positions=[
                    Position(
                        Name="Tester",
                        OrgUnitUuid=first(create_engagements).org_unit,
                        StartDate=datetime(1970, 1, 1),
                        StopDate=None,
                    ),
                    Position(
                        Name="Tester",
                        OrgUnitUuid=first(create_engagements).org_unit,
                        StartDate=datetime(1970, 1, 1),
                        StopDate=None,
                    ),
                ],
                PhoneNumber="11223344",
                Landline=None,
                Email="bsg@example.com",
                Location=None,
                RacfID=None,
                FMKID=None,
                DateTime=None,
            )
        ),
        call(
            User(
                Uuid=adguid2,
                ShortKey=None,
                UserId="AD-username 2",
                Person=Person(Name="Brian Graversen", Cpr=None),
                Positions=[
                    Position(
                        Name="Tester",
                        OrgUnitUuid=first(create_engagements).org_unit,
                        StartDate=datetime(1970, 1, 1),
                        StopDate=None,
                    ),
                    Position(
                        Name="Tester",
                        OrgUnitUuid=first(create_engagements).org_unit,
                        StartDate=datetime(1970, 1, 1),
                        StopDate=None,
                    ),
                ],
                PhoneNumber=None,
                Landline="12345678",
                Email="bsg2@example.com",
                Location=None,
                RacfID=None,
                FMKID=None,
                DateTime=None,
            ),
        ),
    ]
    assert len(os2sync_mock.update_user.call_args_list) == 2
    for c in os2sync_mock.update_user.call_args_list:
        assert c in expected


@pytest.mark.integration_test
async def test_account_with_no_relevant_engagement(
    graphql_client: GraphQLClient,
    create_person,
    create_engagements,
    create_AD_itsystem,
    create_FK_itsystem,
    set_settings,
) -> None:
    # Arrange
    # Set top_unit_uuid to a new uuid to ensure the engagement is not considered relevant.
    settings = set_settings(top_unit_uuid=uuid4())
    adguid = uuid4()
    fk_org_uuid = uuid4()
    os2sync_mock = AsyncMock()
    person_uuid = create_person.uuid
    # AD users
    await graphql_client.testing__ituser_create(
        input=ITUserCreateInput(
            person=person_uuid,
            user_key="AD-username 1",
            external_id=str(adguid),
            engagements=[e.uuid for e in create_engagements],
            itsystem=create_AD_itsystem,
            validity=RAValidityInput(from_=datetime(1970, 1, 1), to=None),  # type: ignore
        )
    )
    # Create an fk-org user and check that it will not be terminated

    fk_account = await graphql_client.testing__ituser_create(
        input=ITUserCreateInput(
            uuid=uuid4(),
            person=person_uuid,
            user_key=str(adguid),
            external_id=str(fk_org_uuid),
            itsystem=create_FK_itsystem,
            validity=RAValidityInput(from_=datetime(1970, 1, 1), to=None),  # type: ignore
        )
    )
    # Act
    await sync_mo_user_to_fk_org(
        uuid=person_uuid,
        graphql_client=graphql_client,
        settings=settings,
        os2sync_client=os2sync_mock,
    )
    # Assert
    os2sync_mock.update_user.assert_not_called()
    os2sync_mock.delete_user.assert_called_once_with(fk_org_uuid)

    # Check that the expected it-accounts exists in MO
    fk_accounts, it_accounts = await read_fk_users_from_person(
        graphql_client=graphql_client,
        uuid=person_uuid,
        it_user_keys=["Active Directory"],
    )
    assert len(it_accounts) == 1
    # Even though the engagement is no longer relevant we keep the fk-org account
    assert fk_accounts == [
        ReadUserITAccountsEmployeesObjectsCurrentFkOrgUuids(
            uuid=fk_account.uuid, user_key=str(adguid), external_id=str(fk_org_uuid)
        )
    ]


@pytest.mark.integration_test
async def test_startdate(
    graphql_client: GraphQLClient,
    mock_settings,
    create_person,
    create_engagements,
    create_AD_itsystem,
    create_FK_itsystem,
    set_settings,
) -> None:
    # Arrange
    adguid = uuid4()
    fk_org_uuid = uuid4()
    os2sync_mock = AsyncMock()
    person_uuid = create_person.uuid

    # Edit the engagements to start earlier
    for e in create_engagements:
        await graphql_client.testing__engagement_update(
            input=EngagementUpdateInput(
                uuid=e.uuid,
                validity=RAValidityInput(from_=datetime(1960, 1, 1)),  # type: ignore
            )
        )
    # AD users
    await graphql_client.testing__ituser_create(
        input=ITUserCreateInput(
            person=person_uuid,
            user_key="AD-username",
            external_id=str(adguid),
            engagements=[e.uuid for e in create_engagements],
            itsystem=create_AD_itsystem,
            validity=RAValidityInput(from_=datetime(1970, 1, 1), to=None),  # type: ignore
        )
    )
    # Create an fk-org user and check that it will not be terminated
    fk_account = await graphql_client.testing__ituser_create(
        input=ITUserCreateInput(
            uuid=uuid4(),
            person=person_uuid,
            user_key=str(adguid),
            external_id=str(fk_org_uuid),
            itsystem=create_FK_itsystem,
            validity=RAValidityInput(from_=datetime(1970, 1, 1), to=None),  # type: ignore
        )
    )
    expected = User(
        Uuid=fk_org_uuid,
        ShortKey=None,
        UserId="AD-username",
        Person=Person(Name="Brian Graversen", Cpr=None),
        Positions=[
            Position(
                Name="Tester",
                OrgUnitUuid=first(create_engagements).org_unit,
                StartDate=datetime(1960, 1, 1),
                StopDate=None,
            ),
            Position(
                Name="Tester",
                OrgUnitUuid=first(create_engagements).org_unit,
                StartDate=datetime(1960, 1, 1),
                StopDate=None,
            ),
        ],
        PhoneNumber=None,
        Landline=None,
        Email=None,
        Location=None,
        RacfID=None,
        FMKID=None,
        DateTime=None,
    )

    # Act
    await sync_mo_user_to_fk_org(
        uuid=person_uuid,
        graphql_client=graphql_client,
        settings=mock_settings,
        os2sync_client=os2sync_mock,
    )
    # Assert
    os2sync_mock.update_user.assert_called_once_with(expected)
    os2sync_mock.delete_user.assert_not_called()

    # Check that the expected it-accounts exists in MO
    fk_accounts, it_accounts = await read_fk_users_from_person(
        graphql_client=graphql_client,
        uuid=person_uuid,
        it_user_keys=["Active Directory"],
    )
    assert len(it_accounts) == 1
    # Even though the engagement is no longer relevant we keep the fk-org account
    assert fk_accounts == [
        ReadUserITAccountsEmployeesObjectsCurrentFkOrgUuids(
            uuid=fk_account.uuid, user_key=str(adguid), external_id=str(fk_org_uuid)
        )
    ]


@pytest.mark.integration_test
async def test_random_uuid(
    graphql_client: GraphQLClient,
    mock_settings,
    create_person,
    create_engagements,
    create_AD_itsystem,
    create_FK_itsystem,
    set_settings,
) -> None:
    # Arrange
    settings = set_settings(randomize_fk_org_uuid=True)
    adguid = uuid4()
    os2sync_mock = AsyncMock()
    # The user does not already exist in fk-org
    # Otherwise we would use that UUID.
    os2sync_mock.os2sync_get_user.side_effect = KeyError()
    person_uuid = create_person.uuid

    # AD users
    await graphql_client.testing__ituser_create(
        input=ITUserCreateInput(
            person=person_uuid,
            user_key="AD-username",
            external_id=str(adguid),
            engagements=[e.uuid for e in create_engagements],
            itsystem=create_AD_itsystem,
            validity=RAValidityInput(from_=datetime(1970, 1, 1), to=None),  # type: ignore
        )
    )

    # Act
    expected_uuid = uuid4()
    with patch("os2sync_export.os2mo_gql.uuid4", return_value=expected_uuid):
        await sync_mo_user_to_fk_org(
            uuid=person_uuid,
            graphql_client=graphql_client,
            settings=settings,
            os2sync_client=os2sync_mock,
        )

    # Assert
    expected = User(
        Uuid=expected_uuid,
        ShortKey=None,
        UserId="AD-username",
        Person=Person(Name="Brian Graversen", Cpr=None),
        Positions=[
            Position(
                Name="Tester",
                OrgUnitUuid=first(create_engagements).org_unit,
                StartDate=datetime(1970, 1, 1),
                StopDate=None,
            ),
            Position(
                Name="Tester",
                OrgUnitUuid=first(create_engagements).org_unit,
                StartDate=datetime(1970, 1, 1),
                StopDate=None,
            ),
        ],
        PhoneNumber=None,
        Landline=None,
        Email=None,
        Location=None,
        RacfID=None,
        FMKID=None,
        DateTime=None,
    )

    os2sync_mock.update_user.assert_called_once_with(expected)
    os2sync_mock.delete_user.assert_not_called()

    # Check that the expected it-accounts exists in MO
    fk_accounts, it_accounts = await read_fk_users_from_person(
        graphql_client=graphql_client,
        uuid=person_uuid,
        it_user_keys=["Active Directory"],
    )
    assert len(it_accounts) == 1
    assert one(fk_accounts).external_id == str(expected_uuid)

    assert one(fk_accounts).user_key == str(adguid)


@pytest.mark.integration_test
async def test_past_account_uuid(
    graphql_client: GraphQLClient,
    mock_settings,
    create_person,
    create_engagements,
    create_AD_itsystem,
    create_FK_itsystem,
    set_settings,
) -> None:
    """Test that we reuse a past fk-org account if the new account has the same account name as a previous account"""
    # Arrange
    settings: Settings = set_settings(randomize_fk_org_uuid=True)
    old_adguid_1 = uuid4()
    old_adguid_2 = uuid4()
    current_adguid = uuid4()
    expected_fk_org_uuid = uuid4()

    os2sync_mock = AsyncMock()
    # The user does not already exist in fk-org
    # Otherwise we would use that UUID.
    os2sync_mock.os2sync_get_user.side_effect = KeyError()
    person_uuid = create_person.uuid

    # AD users -create two in the past and one that is active:
    await graphql_client.testing__ituser_create(
        input=ITUserCreateInput(
            person=person_uuid,
            user_key="AD-username",
            external_id=str(old_adguid_1),
            engagements=[e.uuid for e in create_engagements],
            itsystem=create_AD_itsystem,
            validity=RAValidityInput(
                from_=datetime(1970, 1, 1, 0, 0, 0, tzinfo=UTC),
                to=datetime(1990, 1, 1, 0, 0, 0, tzinfo=UTC),
            ),  # type: ignore
        )
    )
    await graphql_client.testing__ituser_create(
        input=ITUserCreateInput(
            person=person_uuid,
            user_key="AD-username",
            external_id=str(old_adguid_2),
            engagements=[e.uuid for e in create_engagements],
            itsystem=create_AD_itsystem,
            validity=RAValidityInput(
                from_=datetime(1999, 1, 1, 0, 0, 0, tzinfo=UTC),
                to=datetime(2020, 1, 1, 0, 0, 0, tzinfo=UTC),
            ),  # type: ignore
        )
    )
    await graphql_client.testing__ituser_create(
        input=ITUserCreateInput(
            person=person_uuid,
            user_key="AD-username",
            external_id=str(current_adguid),
            engagements=[e.uuid for e in create_engagements],
            itsystem=create_AD_itsystem,
            validity=RAValidityInput(
                from_=datetime(2020, 1, 1, 0, 0, 0, tzinfo=UTC), to=None
            ),  # type: ignore
        )
    )
    # Past FK-org users:
    # The oldest has an fk-org uuid that we ignore
    await graphql_client.testing__ituser_create(
        input=ITUserCreateInput(
            person=person_uuid,
            user_key=str(old_adguid_1),
            external_id=str(uuid4()),
            engagements=[e.uuid for e in create_engagements],
            itsystem=create_FK_itsystem,
            validity=RAValidityInput(
                from_=datetime(1970, 1, 1, 0, 0, 0, tzinfo=UTC),
                to=datetime(1990, 1, 1, 0, 0, 0, tzinfo=UTC),
            ),  # type: ignore
        )
    )
    # This is the latest fk-org uuid, so this is the one we will use.
    await graphql_client.testing__ituser_create(
        input=ITUserCreateInput(
            person=person_uuid,
            user_key=str(old_adguid_2),
            external_id=str(expected_fk_org_uuid),
            engagements=[e.uuid for e in create_engagements],
            itsystem=create_FK_itsystem,
            validity=RAValidityInput(
                from_=datetime(2010, 1, 1, 0, 0, 0, tzinfo=UTC),
                to=datetime(2020, 1, 1, 0, 0, 0, tzinfo=UTC),
            ),  # type: ignore
        )
    )

    # Act
    await sync_mo_user_to_fk_org(
        uuid=person_uuid,
        graphql_client=graphql_client,
        settings=settings,
        os2sync_client=os2sync_mock,
    )

    # Assert
    expected = User(
        Uuid=expected_fk_org_uuid,
        ShortKey=None,
        UserId="AD-username",
        Person=Person(Name="Brian Graversen", Cpr=None),
        Positions=[
            Position(
                Name="Tester",
                OrgUnitUuid=first(create_engagements).org_unit,
                StartDate=datetime(1970, 1, 1, 0, 0, 0, tzinfo=UTC),
                StopDate=None,
            ),
            Position(
                Name="Tester",
                OrgUnitUuid=first(create_engagements).org_unit,
                StartDate=datetime(1970, 1, 1, 0, 0, 0, tzinfo=UTC),
                StopDate=None,
            ),
        ],
        PhoneNumber=None,
        Landline=None,
        Email=None,
        Location=None,
        RacfID=None,
        FMKID=None,
        DateTime=None,
    )

    os2sync_mock.update_user.assert_called_once_with(expected)
    os2sync_mock.delete_user.assert_not_called()

    # Check that the expected it-accounts exists in MO
    fk_accounts, it_accounts = await read_fk_users_from_person(
        graphql_client=graphql_client,
        uuid=person_uuid,
        it_user_keys=settings.it_system_user_keys,
    )
    assert len(it_accounts) == 1
    assert one(fk_accounts).external_id == str(expected_fk_org_uuid)

    assert one(fk_accounts).user_key == str(current_adguid)


@pytest.mark.integration_test
async def test_future_engagement(
    graphql_client: GraphQLClient,
    mock_settings,
    create_person,
    create_org_unit,
    create_job_function,
    create_engagement_type,
    create_AD_itsystem,
    create_FK_itsystem,
    set_settings,
) -> None:
    # Arrange
    # Create an engagement that starts in the future, link it to a current AD account and assert that it would be written to os2sync
    engagement_startdate = datetime.now() + timedelta(days=30)
    adguid = uuid4()
    os2sync_mock = AsyncMock()
    # Mock that user is not in os2sync already
    os2sync_mock.os2sync_get_user.side_effect = [KeyError]
    person_uuid = create_person.uuid
    res = await graphql_client.testing__engagement_create(
        input=EngagementCreateInput(
            employee=create_person.uuid,
            org_unit=create_org_unit.uuid,
            user_key="1234",
            engagement_type=create_engagement_type,
            job_function=create_job_function,
            validity=RAValidityInput(from_=engagement_startdate, to=None),  # type: ignore
        )
    )
    engagement_uuid = res.uuid
    # AD users
    await graphql_client.testing__ituser_create(
        input=ITUserCreateInput(
            person=person_uuid,
            user_key="AD-username",
            external_id=str(adguid),
            engagements=[engagement_uuid],
            itsystem=create_AD_itsystem,
            validity=RAValidityInput(from_=datetime(1970, 1, 1), to=None),  # type: ignore
        )
    )

    expected = User(
        Uuid=adguid,
        ShortKey=None,
        UserId="AD-username",
        Person=Person(Name="Brian Graversen", Cpr=None),
        Positions=[
            Position(
                Name="Tester",
                OrgUnitUuid=create_org_unit.uuid,
                StartDate=engagement_startdate,
                StopDate=None,
            ),
        ],
        PhoneNumber=None,
        Landline=None,
        Email=None,
        Location=None,
        RacfID=None,
        FMKID=None,
        DateTime=None,
    )

    # Act
    await sync_mo_user_to_fk_org(
        uuid=person_uuid,
        graphql_client=graphql_client,
        settings=mock_settings,
        os2sync_client=os2sync_mock,
    )
    # Assert
    os2sync_mock.update_user.assert_called_once_with(expected)
    os2sync_mock.delete_user.assert_not_called()

    # Check that the expected it-accounts exists in MO
    fk_accounts, it_accounts = await read_fk_users_from_person(
        graphql_client=graphql_client,
        uuid=person_uuid,
        it_user_keys=["Active Directory"],
    )
    assert len(it_accounts) == 1
    # Even though the engagement is no longer relevant we keep the fk-org account
    assert one(fk_accounts).user_key == str(adguid)
    assert one(fk_accounts).external_id == str(adguid)
