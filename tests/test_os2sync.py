...
@pytest.mark.asyncio
async def test_os2sync_upsert_org_unit_no_changes(mock_os2sync_client, respx_mock):
    """Test that if there are no changes to an org_unit we won't write to os2sync"""

    mock_os2sync_client.os2sync_get_org_unit = AsyncMock(return_value=o)

    respx_mock.post(
        f"{mock_os2sync_client.settings.os2sync_api_url}/orgUnit/",
        json=jsonable_encoder(o),
    ).mock(return_value=httpx.Response(200))
    await mock_os2sync_client.upsert_org_unit(o)


@pytest.mark.asyncio
async def test_os2sync_upsert_org_unit_new(mock_os2sync_client, respx_mock):
    """Test that if no orgUnit was found in fk-org we create it."""

    mock_os2sync_client.os2sync_get_org_unit = AsyncMock(side_effect=KeyError)
...
@pytest.mark.asyncio
async def test_os2sync_upsert_org_unit_changes(mock_os2sync_client, respx_mock):
    """If there are changes to an orgunit it is sent to os2sync"""
    org_unit = o.copy()

    mock_os2sync_client.os2sync_get_org_unit = AsyncMock(return_value=o.copy())
    ...
    await mock_os2sync_client.upsert_org_unit(org_unit)


@pytest.mark.asyncio
async def test_os2sync_upsert_org_unit_keep_fk_fields(mock_os2sync_client, respx_mock):
    """Test that certain fields are fetched from fk-org. If these fields are found we use their values in the payload"""

    mock_os2sync_client.os2sync_get_org_unit = AsyncMock(return_value=o2)
...
@pytest.mark.asyncio
async def test_os2sync_upsert_org_unit_changes_w_fixed_fields(
    ...
    mock_os2sync_client.os2sync_get_org_unit = AsyncMock(return_value=fk_org)
...
@pytest.mark.asyncio
async def test_os2sync_upsert_org_unit_ordered_tasks(mock_os2sync_client, respx_mock):
    ...
    mock_os2sync_client.os2sync_get_org_unit = AsyncMock(return_value=current)
    respx_mock.post(
        f"{mock_os2sync_client.settings.os2sync_api_url}/orgUnit/",
        json=jsonable_encoder(org_unit),
    ).mock(return_value=httpx.Response(200))
...
@pytest.mark.asyncio
async def test_update_orgunit_upsert(mock_os2sync_client):
    mock_os2sync_client.upsert_org_unit = AsyncMock()
    await mock_os2sync_client.update_org_unit(o.Uuid, o)
    mock_os2sync_client.upsert_org_unit.assert_called_with(o)


@pytest.mark.asyncio
async def test_update_orgunit_delete(mock_os2sync_client):
    mock_os2sync_client.delete_orgunit = AsyncMock()
    await mock_os2sync_client.update_org_unit(o.Uuid, None)
    mock_os2sync_client.delete_orgunit.assert_called_with(o.Uuid)


@pytest.mark.asyncio
async def test_update_users(mock_os2sync_client):
    mock_os2sync_client.os2sync_post = AsyncMock()
    await mock_os2sync_client.update_users(uuid, [u1, u2])
...
@pytest.mark.asyncio
async def test_update_users_one_overwritten_uuid_account(mock_os2sync_client):
    mock_os2sync_client.os2sync_delete = AsyncMock()
    mock_os2sync_client.os2sync_post = AsyncMock()
    users = [u1, u2]
    await mock_os2sync_client.update_users(user_uuid, users)
    mock_os2sync_client.os2sync_delete.assert_not_called()


@pytest.mark.asyncio
async def test_update_users_overwritten_uuid_account(mock_os2sync_client):
    mock_os2sync_client.os2sync_delete = AsyncMock()
    mock_os2sync_client.os2sync_post = AsyncMock()
    users = [u2]
    await mock_os2sync_client.update_users(user_uuid, users)
...
@pytest.mark.asyncio
async def test_update_users_no_user(mock_os2sync_client):
    mock_os2sync_client.os2sync_delete = AsyncMock()
    users = []
    await mock_os2sync_client.update_users(user_uuid, users)
...
async def test_get_hierarchy(
    input_org_units, input_users, mock_os2sync_client, respx_mock
):
    ...
    # Act
    org_units, users = await mock_os2sync_client.get_hierarchy(
        request_uuid=request_uuid
    )
    assert org_units == input_org_units
    assert users == input_users

    org_units, users = await mock_os2sync_client.get_existing_uuids(
        request_uuid=request_uuid
    )
...
@pytest.mark.asyncio
async def test_get_hierarchy_retry(mock_settings, mock_os2sync_client, respx_mock):
    ...
    with patch("time.sleep"):
        await mock_os2sync_client.get_existing_uuids(request_uuid)

    assert route.call_count == 3


@pytest.mark.asyncio
async def test_update_users_delete_uuid_from_it_user(mock_settings):
    ...
    await client.update_users(uuid=uuid4(), users=users)

    client.delete_user.assert_called_once_with(uuid)


@pytest.mark.asyncio
async def test_update_user(mock_settings, respx_mock):
    client = WritableOS2SyncClient(mock_settings, None)
    user = User(
        **{
            "Uuid": uuid4(),
    ...
    await client.update_user(user=user)
...
@pytest.mark.asyncio
async def test_dry_run_delete_orgunit(dry_run, mock_settings, respx_mock):
    org_unit_uuid = uuid4()
    ...
    await client.delete_orgunit(uuid=org_unit_uuid)


@pytest.mark.parametrize("dry_run", (True, False))
@pytest.mark.asyncio
async def test_dry_run_update_orgunit(dry_run, mock_settings, respx_mock):
    org_unit_uuid = uuid4()
    client = get_os2sync_client(mock_settings, session=None, dry_run=dry_run)

    # Fake an orgunit wasn't found in fk-org
    client.os2sync_get_org_unit = AsyncMock(side_effect=KeyError)
    ...
    await client.update_org_unit(org_unit_uuid, orgunit)


@pytest.mark.parametrize("dry_run", (True, False))
@pytest.mark.asyncio
async def test_dry_run_delete_user(dry_run, mock_settings, respx_mock):
    user_uuid = uuid4()
    client = get_os2sync_client(mock_settings, session=None, dry_run=dry_run)
    ...
    await client.delete_user(uuid=user_uuid)


@pytest.mark.parametrize("dry_run", (True, False))
@pytest.mark.asyncio
async def test_dry_run_update_user(dry_run, mock_settings, respx_mock):
    user_uuid = uuid4()
    client = get_os2sync_client(mock_settings, session=None, dry_run=dry_run)
    user = User(
        Uuid=user_uuid,
