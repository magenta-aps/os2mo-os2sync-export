...
from os2sync_export.autogenerated_graphql_client.fragments import UnitFieldsItusers
...
from os2sync_export.autogenerated_graphql_client.read_user_i_t_accounts import (
    ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementsResponsesObjectsValiditiesJobFunction as JobFunction,
)
from os2sync_export.autogenerated_graphql_client.read_user_i_t_accounts import (
...
BASE_UNIT = ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementsResponsesObjectsValiditiesOrgUnit(
    uuid=TOP_UNIT_UUID,
    itusers=[],
    unit_type=None,
    org_unit_level=None,
...
FIRST_STARTDATE = datetime(2017, 5, 7)
BASE_ITUSER_RESPONSE = ReadUserITAccountsEmployeesObjectsCurrentItusers(
    ...
    engagements_responses=ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementsResponses(
        objects=[
            ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementsResponsesObjects(
                validities=[
                    ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementsResponsesObjectsValidities(
                ...
                startdates=[
                    ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementsResponsesObjectsStartdates(
                        validity=ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementsResponsesObjectsStartdatesValidity(
                            from_=datetime(2021, 2, 1)  # type: ignore
                        )
                    ),
                    ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementsResponsesObjectsStartdates(
                        validity=ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementsResponsesObjectsStartdatesValidity(
                            from_=FIRST_STARTDATE  # type: ignore
                        )
                    ),
                ],
            )
...
async def test_sync_mo_user_to_fk_empty_it_accounts(
    mock_graphql_client, mock_settings, os2sync_client
):
    """Test that an empty it_accounts objects list doesn't crash"""
    mock_graphql_client.find_f_k_itsystem.return_value = FindFKItsystemItsystems(
        **{"objects": [{"uuid": uuid4()}]}
    )
    mock_graphql_client.read_user_i_t_accounts.return_value = (
        ReadUserITAccountsEmployees(objects=[])
    )
    uuid = uuid4()
    await sync_mo_user_to_fk_org(
        graphql_client=mock_graphql_client,
...
def test_convert_to_os2sync(mock_settings):
    ...
    assert os2sync_user.Positions[0].StartDate == FIRST_STARTDATE.date()
    assert os2sync_user.Positions[0].StopDate is None
    assert os2sync_user.Positions[0].Name == "tester"
...
@pytest.mark.parametrize(
    ...
    [
        (ExtensionField.EXTENSION_3, "Konge"),
        (ExtensionField.EXTENSION_1, "Regent"),
        (None, "Udvikler"),
    ],
)
...
def test_convert_to_os2sync_engagement_org_unit_uuid(mock_settings):
    ...
    org_unit.itusers = [UnitFieldsItusers(user_key=str(fk_org_unit_uuid))]
    mo_it_user.engagements_responses = ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementsResponses(
        objects=[
            ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementsResponsesObjects(
                validities=[
...
def test_convert_to_os2sync_future_engagement(mock_settings):
        ...
        objects=[
            ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementsResponsesObjects(
                validities=[
                    ...
                    ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementsResponsesObjectsValidities(
                        extension_3="Galakse-Kejser",
                        org_unit=[org_unit],
                        job_function=JobFunction(name="Rockstar-Udvikler"),
                    ),
                ],
                startdates=[
                    ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementsResponsesObjectsStartdates(
                        validity=ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementsResponsesObjectsStartdatesValidity(
                            from_=start_date, to=start_date + timedelta(days=10)
                        )
                    ),
                    ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementsResponsesObjectsStartdates(
                        validity=ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementsResponsesObjectsStartdatesValidity(
                            from_=start_date + timedelta(days=10)
                        )
                    ),
                ],
            )
    ...
    assert os2sync_user.Positions[0].OrgUnitUuid == fk_org_unit_uuid
    assert os2sync_user.Positions[0].StartDate == start_date.date()
...
def test_convert_and_filter_no_engagement(mock_settings):
    ...
    it_user.engagements_responses = (
        ReadUserITAccountsEmployeesObjectsCurrentItusersEngagementsResponses(objects=[])
    )
