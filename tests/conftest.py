...
async def create_org_unit(
    graphql_client: GraphQLClient, mock_settings
) -> TestingOrgUnitCreateOrgUnitCreate:
    org_unit_type_facet = (
...
@pytest.fixture
async def create_engagement_classes(
    graphql_client: GraphQLClient,
) -> tuple[UUID, UUID]:
    engagemnt_type_facet = (
        await graphql_client.testing__create_facet(
            input=FacetCreateInput(
    ...
    engagement_type = (
        await graphql_client.testing__create_class(
            input=ClassCreateInput(
                name="ansat",
                user_key="-",
                facet_uuid=engagemnt_type_facet,
                validity=ValidityInput(from_=None, to=None),  # type: ignore
            )
        )
    ).uuid
    ...
    return engagement_type, job_function
